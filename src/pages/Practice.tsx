...
import { fetchTopics, startSession, fetchSessionQuestions, submitResponse, completeSession, type PracticeTopic, type PracticeQuestion, type SubmitResult, type SessionSummary } from '../lib/practice'
...
export default function Practice({ onBack }: { onBack: () => void }) {
  ...
  const beginTopic = async (topicId: string) => {
    setError(''); setView({ stage: 'loading_session' })
    try {
      const sessionId = await startSession(topicId)
      const questions = await fetchSessionQuestions(sessionId)
      if (questions.length === 0) { setError('No questions are available for this topic yet.'); setView({ stage: 'topics' }); return }
      setView({ stage: 'in_session', sessionId, questions, index: 0, questionStartedAt: Date.now(), result: null, selected: null })
    } catch (e: any) { setError(e.message ?? 'Could not start a practice session.'); setView({ stage: 'topics' }) }
  }
  ...
  const pickAnswer = async (choice: string) => {
    ...
    } catch (e: any) { setError(e.message ?? 'Could not submit that answer.') }
  }

  const nextQuestion = async () => {
    if (view.stage !== 'in_session') return
    if (view.index + 1 >= view.questions.length) {
      try { setView({ stage: 'complete', summary: await completeSession(view.sessionId) }) }
      catch (e: any) { setError(e.message ?? 'Could not finish the session.') }
      return
    }
    setView({ ...view, index: view.index + 1, questionStartedAt: Date.now(), result: null, selected: null })
  }

  if (view.stage === 'topics') return <div className="dash-main">
    <div className="section-heading"><h1>Practice</h1><button className="btn btn-secondary" onClick={onBack}>Back to dashboard</button></div>
    {error && <div className="banner banner-error">{error}</div>}
    {recommendations.length > 0 && <>
      <div className="section-heading"><h2>Recommended for you</h2></div>
      <div className="list-panel">{recommendations.slice(0, 3).map(r => <div className="list-row" key={r.topic_id}>
        <div><div className="list-row-title">{r.topic}</div><div className="list-row-meta">{r.subject}</div></div>
    ...
    {topics && topics.length > 0 && <div className="list-panel">{topics.map(t => <div className="list-row" key={t.id}>
      <div><div className="list-row-title">{t.topic}</div><div className="list-row-meta">{t.subject}{t.grade_level ? ` · Grade ${t.grade_level}` : ''}</div></div>
      <button className="btn btn-primary" onClick={() => beginTopic(t.id)}>Start</button>
    </div>)}</div>}
  </div>

  if (view.stage === 'loading_session') return <div className="centered-loading">Setting up your practice session…</div>
  if (view.stage === 'in_session') {
    const q = view.questions[view.index]
    return <div className="dash-main">
      <div className="section-heading"><h2>Question {view.index + 1} of {view.questions.length}</h2></div>
      {error && <div className="banner banner-error">{error}</div>}
      <p style={{ fontSize: '1.1rem', fontWeight: 600 }}>{q.question}</p>
      <div className="list-panel">{(q.choices ?? []).map(choice => {
        const showResult = view.result && view.selected === choice
        return <button key={choice} className="list-row" style={{ width: '100%', border: 'none', textAlign: 'left', cursor: view.result ? 'default' : 'pointer', background: showResult ? (view.result?.is_correct ? '#eaf5ee' : '#fbeceb') : 'transparent' }} onClick={() => pickAnswer(choice)} disabled={!!view.result}>
          <span className="list-row-title">{choice}</span>
        </button>
      })}</div>
      {view.result && <div className={`banner ${view.result.is_correct ? 'banner-info' : 'banner-error'}`}>{view.result.is_correct ? 'Correct.' : 'Not quite.'} {view.result.feedback}</div>}
      {view.result && <button className="btn btn-primary" onClick={nextQuestion}>{view.index + 1 >= view.questions.length ? 'Finish session' : 'Next question'}</button>}
    </div>
  }
  return <div className="dash-main"><h1>Session complete</h1><div className="stat-strip">
  ...
  </div><button className="btn btn-primary" onClick={onBack}>Back to dashboard</button></div>
}
